...
// #[debug_handler]
pub async fn find_companies(State(state): State<AppState>) -> Result<Json<Vec<CompanyWrapper>>, AppError> {
    let dao = state.company_dao.read().await;
    let result = dao.find().await?;
    let result2 = result.into_iter().map(|x| CompanyWrapper(x)).collect();
    Ok(Json(result2))
}
